...
class ADExplorerSnapshot(object):
    ...
    def preprocessCached(self):
        cacheFileName = hashlib.md5(
            f"{self.snap.header.filetime}_{self.snap.header.server}".encode()).hexdigest() + ".cache"
    ...
    def preprocess(self):
        ...
        for idx, obj in enumerate(self.snap.objects):
            ...
            # get computers
            if ADUtils.get_entry_property(obj, 'sAMAccountType', -1) == 805306369 and not (
                    ADUtils.get_entry_property(obj, 'userAccountControl', 0) & 0x02 == 0x02):
            ...
            if self.log and self.log.term_mode:
                prog.status(
                    f"{idx + 1}/{self.snap.header.numObjects} ({len(self.sidcache)} sids, {len(self.computersidcache)} computers, {len(self.domains)} domains with {len(self.domaincontrollers)} DCs)")

        if self.log:
            prog.success(
                f"{len(self.sidcache)} sids, {len(self.computersidcache)} computers, {len(self.domains)} domains with {len(self.domaincontrollers)} DCs")
    ...
    def process(self):
        ...
        for ptype in ['users', 'computers', 'groups', 'domains']:
            ...
            results_worker = threading.Thread(target=OutputWorker.membership_write_worker, args=(
            self.writeQueues[ptype], ptype,
            os.path.join(self.output, f"{self.snap.header.server}_{self.snap.header.filetimeUnix}_{ptype}.json")))
            results_worker.daemon = True
            results_worker.start()
        ...
        for idx, obj in enumerate(self.snap.objects):
            ...
            if self.log and self.log.term_mode:
                prog.status(
                    f"{idx + 1}/{self.snap.header.numObjects} ({self.numUsers} users, {self.numGroups} groups, {self.numComputers} computers, {self.numTrusts} trusts)")

        if self.log:
            prog.success(
                f"{self.numUsers} users, {self.numGroups} groups, {self.numComputers} computers, {self.numTrusts} trusts")
        ...
        if self.log:
            self.log.success(
                f"Output written to {self.snap.header.server}_{self.snap.header.filetimeUnix}_*.json files")
    ...
    def processDomains(self):
        ...
        aces = self.parse_acl(domain, 'domain',
                              ADUtils.get_entry_property(self.domain_object, 'nTSecurityDescriptor', raw=True))
    ...
    def processComputers(self, entry):
        if not ADUtils.get_entry_property(entry, 'sAMAccountType', -1) == 805306369 or (
                ADUtils.get_entry_property(entry, 'userAccountControl', 0) & 0x02 == 0x02):
            return
        ...
        props['unconstraineddelegation'] = ADUtils.get_entry_property(entry, 'userAccountControl',
                                                                      default=0) & 0x00080000 == 0x00080000
        props['enabled'] = ADUtils.get_entry_property(entry, 'userAccountControl', default=0) & 2 == 0
        props['trustedtoauth'] = ADUtils.get_entry_property(entry, 'userAccountControl',
                                                            default=0) & 0x01000000 == 0x01000000
        ...
        props['sidhistory'] = [LDAP_SID(bsid).formatCanonical() for bsid in
                               ADUtils.get_entry_property(entry, 'sIDHistory', [])]
        ...
        aces = self.parse_acl(computer, 'computer',
                              ADUtils.get_entry_property(entry, 'msDS-AllowedToActOnBehalfOfOtherIdentity', raw=True))
    ...
    def processTrusts(self, entry):
        ...
        domtrust = ADDomainTrust(ADUtils.get_entry_property(entry, 'name'),
    ...
    def processUsers(self, entry):
        if not ((
                        'user' in entry.classes and 'person' == entry.category) or 'msds-groupmanagedserviceaccount' in entry.classes):
            return
        ...
        user = {
            ...
            "Properties": {
                ...
                "domainsid": self.domainsid,
                "distinguishedname": distinguishedName,
                "unconstraineddelegation": ADUtils.get_entry_property(entry, 'userAccountControl',
                                                                      default=0) & 0x00080000 == 0x00080000,
                "trustedtoauth": ADUtils.get_entry_property(entry, 'userAccountControl',
                                                            default=0) & 0x01000000 == 0x01000000,
                "passwordnotreqd": ADUtils.get_entry_property(entry, 'userAccountControl',
                                                              default=0) & 0x00000020 == 0x00000020
            },
            "Aces": [],
            "SPNTargets": [],
            "HasSIDHistory": [],
    ...
    def parse_acl(self, entry, entrytype, acl):
        parselaps = entrytype == 'computer' and entry['Properties'][
            'haslaps'] and "ms-mcs-admpwd" in self.objecttype_guid_map
...
def main():
    parser = argparse.ArgumentParser(add_help=True, description='AD Explorer snapshot ingestor for BloodHound',
    ...
    parser.add_argument('-o', '--output', required=False, type=pathlib.Path,
                        help="Path to the *.json output folder. Folder will be created if it doesn't exist. Defaults to the current directory.",
                        default=".")
    parser.add_argument('-m', '--mode', required=False,
                        help="The output mode to use. Besides BloodHound JSON output files, it is possible to dump all objects with all attributes to NDJSON. Defaults to BloodHound output mode.",
                        choices=ADExplorerSnapshot.OutputMode.__members__, default='BloodHound')
